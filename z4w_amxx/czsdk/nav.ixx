...
export namespace czsdk {

// to help identify nav files
constexpr auto NAV_MAGIC_NUMBER = 0xFEEDFACE;

// A place is a named group of navigation areas
typedef unsigned int Place;

// ie: "no place"
constexpr auto UNDEFINED_PLACE = 0;
constexpr auto ANY_PLACE = 0xFFFF;

constexpr auto WALK_THRU_DOORS = 0x01;
constexpr auto WALK_THRU_BREAKABLES = 0x02;
constexpr auto WALK_THRU_EVERYTHING = (WALK_THRU_DOORS | WALK_THRU_BREAKABLES);
...
{
	int cx = static_cast<int>(pos->x / GenerationStepSize);
	int cy = static_cast<int>(pos->y / GenerationStepSize);
...
{
	int c = static_cast<int>(*value / GenerationStepSize);
