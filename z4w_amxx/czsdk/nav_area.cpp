...
module czsdk.nav_area;

import hlsdk.cvardef;
import hlsdk.extdll;
import hlsdk.util;

import czsdk.nav_node;
import czsdk.bot_util;
import czsdk.steam_util;
...
EngineClock::time_point lastDrawTimestamp = {};
...
bool CNavArea::MergeEdit(CNavArea *adj)
{
	...
	bool merge = false;
	if (std::abs(m_extent.lo.x - adj->m_extent.lo.x) < tolerance &&
		std::abs(m_extent.hi.x - adj->m_extent.hi.x) < tolerance)
		merge = true;

	if (std::abs(m_extent.lo.y - adj->m_extent.lo.y) < tolerance &&
		std::abs(m_extent.hi.y - adj->m_extent.hi.y) < tolerance)
		merge = true;
