...
export namespace hlsdk
{
	...
	// edict->flags
	constexpr auto FL_FLY = (1 << 0); // Changes the SV_Movestep() behavior to not need to be on ground
	constexpr auto FL_SWIM = (1 << 1); // Changes the SV_Movestep() behavior to not need to be on ground (but stay in water)
	constexpr auto FL_CONVEYOR = (1 << 2);
	constexpr auto FL_CLIENT = (1 << 3);
	constexpr auto FL_INWATER = (1 << 4);
	constexpr auto FL_MONSTER = (1 << 5);
	constexpr auto FL_GODMODE = (1 << 6);
	constexpr auto FL_NOTARGET = (1 << 7);
	constexpr auto FL_SKIPLOCALHOST = (1 << 8); // Don't send entity to local host, it's predicting this entity itself
	constexpr auto FL_ONGROUND = (1 << 9); // At rest / on the ground
	constexpr auto FL_PARTIALGROUND = (1 << 10); // not all corners are valid
	constexpr auto FL_WATERJUMP = (1 << 11); // player jumping out of water
	constexpr auto FL_FROZEN = (1 << 12); // Player is frozen for 3rd person camera
	constexpr auto FL_FAKECLIENT = (1 << 13); // JAC: fake client, simulated server side; don't send network messages to them
	constexpr auto FL_DUCKING = (1 << 14); // Player flag -- Player is fully crouched
	constexpr auto FL_FLOAT = (1 << 15); // Apply floating force to this entity when in water
	constexpr auto FL_GRAPHED = (1 << 16); // worldgraph has this ent listed as something that blocks a connection

	// UNDONE: Do we need these?
	constexpr auto FL_IMMUNE_WATER = (1 << 17);
	constexpr auto FL_IMMUNE_SLIME = (1 << 18);
	constexpr auto FL_IMMUNE_LAVA = (1 << 19);
	constexpr auto FL_PROXY = (1 << 20); // This is a spectator proxy
	constexpr auto FL_ALWAYSTHINK = (1 << 21); // Brush model flag -- call think every frame regardless of nextthink - ltime (for constantly changing velocity/path)
	constexpr auto FL_BASEVELOCITY = (1 << 22); // Base velocity has been applied this frame (used to convert base velocity into momentum)
	constexpr auto FL_MONSTERCLIP = (1 << 23); // Only collide in with monsters who have FL_MONSTERCLIP set
	constexpr auto FL_ONTRAIN = (1 << 24); // Player is _controlling_ a train, so movement commands should be ignored on client during prediction.
	constexpr auto FL_WORLDBRUSH = (1 << 25); // Not moveable/removeable brush entity (really part of the world, but represented as an entity for transparency or something)
	constexpr auto FL_SPECTATOR = (1 << 26); // This client is a spectator, don't run touch functions, etc.
	constexpr auto FL_CUSTOMENTITY = (1 << 29); // This is a custom entity
	constexpr auto FL_KILLME = (1 << 30); // This entity is marked for death -- This allows the engine to kill ents at the appropriate time
	constexpr auto FL_DORMANT = (1 << 31); // Entity is dormant, no updates to client

	// Goes into globalvars_t.trace_flags
	constexpr auto FTRACE_SIMPLEBOX = (1 << 0); // Traceline with a simple box
	constexpr auto FTRACE_IGNORE_GLASS = (1 << 1); // traceline will be ignored entities with rendermode != kRenderNormal

	// walkmove modes
	constexpr auto WALKMOVE_NORMAL = 0; // normal walkmove
	constexpr auto WALKMOVE_WORLDONLY = 1; // doesn't hit ANY entities, no matter what the solid type
	constexpr auto WALKMOVE_CHECKONLY = 2; // move, but don't touch triggers

	// edict->movetype values
	constexpr auto MOVETYPE_NONE = 0; // never moves
	constexpr auto MOVETYPE_ANGLENOCLIP = 1;
	constexpr auto MOVETYPE_ANGLECLIP = 2;
	constexpr auto MOVETYPE_WALK = 3; // Player only - moving on the ground
	constexpr auto MOVETYPE_STEP = 4; // gravity, special edge handling -- monsters use this
	constexpr auto MOVETYPE_FLY = 5; // No gravity, but still collides with stuff
	constexpr auto MOVETYPE_TOSS = 6; // gravity/collisions
	constexpr auto MOVETYPE_PUSH = 7; // no clip to world, push and crush
	constexpr auto MOVETYPE_NOCLIP = 8; // No gravity, no collisions, still do velocity/avelocity
	constexpr auto MOVETYPE_FLYMISSILE = 9; // extra size to monsters
	constexpr auto MOVETYPE_BOUNCE = 10; // Just like Toss, but reflect velocity when contacting surfaces
	constexpr auto MOVETYPE_BOUNCEMISSILE = 11; // bounce w/o gravity
	constexpr auto MOVETYPE_FOLLOW = 12; // track movement of aiment
	constexpr auto MOVETYPE_PUSHSTEP = 13; // BSP model that needs physics/world collisions (uses nearest hull for world collision)
	constexpr auto MOVETYPE_COMPOUND = 14; // glue two entities together (simple movewith)

	// edict->solid values
	// NOTE: Some movetypes will cause collisions independent of SOLID_NOT/SOLID_TRIGGER when the entity moves
	// SOLID only effects OTHER entities colliding with this one when they move - UGH!
	constexpr auto SOLID_NOT = 0; // no interaction with other objects
	constexpr auto SOLID_TRIGGER = 1; // touch on edge, but not blocking
	constexpr auto SOLID_BBOX = 2; // touch on edge, block
	constexpr auto SOLID_SLIDEBOX = 3; // touch on edge, but not an onground
	constexpr auto SOLID_BSP = 4; // bsp clip, touch on edge, block
	constexpr auto SOLID_CUSTOM = 5; // call external callbacks for tracing

	// edict->deadflag values
	constexpr auto DEAD_NO = 0; // alive
	constexpr auto DEAD_DYING = 1; // playing death animation or still falling off of a ledge waiting to hit ground
	constexpr auto DEAD_DEAD = 2; // dead. lying still.
	...
	// entity effects
	constexpr auto EF_BRIGHTFIELD = 1; // swirling cloud of particles
	constexpr auto EF_MUZZLEFLASH = 2; // single frame ELIGHT on entity attachment 0
	constexpr auto EF_BRIGHTLIGHT = 4; // DLIGHT centered at entity origin
	constexpr auto EF_DIMLIGHT = 8; // player flashlight
	constexpr auto EF_INVLIGHT = 16; // get lighting from ceiling
	constexpr auto EF_NOINTERP = 32; // don't interpolate the next frame
	constexpr auto EF_LIGHT = 64; // rocket flare glow sprite
	constexpr auto EF_NODRAW = 128; // don't draw entity



	constexpr auto EF_NOREFLECT = (1 << 24); // Entity won't reflecting in mirrors
	constexpr auto EF_REFLECTONLY = (1 << 25); // Entity will be drawing only in mirrors
	constexpr auto EF_NOWATERCSG = (1 << 26); // Do not remove sides for func_water entity
	constexpr auto EF_FULLBRIGHT = (1 << 27); // Just get fullbright
	constexpr auto EF_NOSHADOW = (1 << 28); // ignore shadow for this entity
	constexpr auto EF_MERGE_VISIBILITY = (1 << 29); // this entity allowed to merge vis (e.g. env_sky or portal camera)
	constexpr auto EF_REQUEST_PHS = (1 << 30); // This entity requested phs bitvector instead of pvsbitvector in AddToFullPack calls
	// g-cont. one reserved bit here for me

	// entity flags
	constexpr auto EFLAG_SLERP = 1; // do studio interpolation of this entity
	...
	constexpr auto TE_EXPLFLAG_DRAWALPHA = 16; // sprite will be drawn alpha
	constexpr auto TE_EXPLFLAG_ROTATE = 32; // rotate the sprite randomly
	...
	constexpr auto TE_BEAM = 16; // obsolete
	...
	constexpr auto TE_BEAMHOSE = 26; // obsolete
	...
	constexpr auto TE_SPRITE_SPRAY = 110; // spay of alpha sprites
	// coord, coord, coord (position)
	// coord, coord, coord (velocity)
	// short (sprite index)
	// byte (count)
	...
	// From HLSDK 2.4
	constexpr auto TEFIRE_FLAG_ADDITIVE = 32; // if set, sprite is rendered non-opaque with additive
	...
	constexpr auto MSG_BROADCAST = 0; // unreliable to all
	constexpr auto MSG_ONE = 1; // reliable to one (msg_entity)
	constexpr auto MSG_ALL = 2; // reliable to all
	constexpr auto MSG_INIT = 3; // write to the init string
	constexpr auto MSG_PVS = 4; // Ents in PVS of org
	constexpr auto MSG_PAS = 5; // Ents in PAS of org
	constexpr auto MSG_PVS_R = 6; // Reliable to PVS
	constexpr auto MSG_PAS_R = 7; // Reliable to PAS
	constexpr auto MSG_ONE_UNRELIABLE = 8; // Send to one client, but don't put in reliable stream, put in unreliable datagram ( could be dropped )
	constexpr auto MSG_SPEC = 9; // Sends to all spectator proxies
	...
	constexpr auto CONTENTS_ORIGIN = -7; // removed at csg time
	constexpr auto CONTENTS_CLIP = -8; // changed to contents_solid
	...
	constexpr auto CHAN_STREAM = 5; // allocate stream channel from the static or dynamic area
	constexpr auto CHAN_STATIC = 6; // allocate channel from the static area 
	constexpr auto CHAN_NETWORKVOICE_BASE = 7; // voice data coming across the network
	constexpr auto CHAN_NETWORKVOICE_END = 500; // network voice data reserves slots (CHAN_NETWORKVOICE_BASE through CHAN_NETWORKVOICE_END).

	// From HLSDK 2.4
	constexpr auto CHAN_BOT = 501; // channel used for bot chatter

	// attenuation values
	constexpr auto ATTN_NONE = 0;
	constexpr auto ATTN_NORM = (float)0.8;
	constexpr auto ATTN_IDLE = (float)2;
	constexpr auto ATTN_STATIC = (float)1.25;
	// pitch values
	constexpr auto PITCH_NORM = 100; // non-pitch shifted
	...
	constexpr auto SF_TRAIN_START_ON = 4; // Train is initially moving
	constexpr auto SF_TRAIN_PASSABLE = 8; // Train is not solid -- used to make water trains
	...
	constexpr auto IN_SCORE = (1 << 15); // Used by client.dll for when scoreboard is held down
